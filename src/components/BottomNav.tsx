  ...
  const renderItem = (item: { name: string; href: string; icon: React.ElementType }) => {
    ...
    return (
      <Link
        ...
        href={item.href}
        className={cn(
          'flex flex-col items-center justify-center w-full flex-1 gap-1 text-muted-foreground transition-colors py-2',
          isActive && 'text-primary'
        )}
      >
        <item.icon
          className={cn('h-[22px] w-[22px]', isActive && 'fill-primary/20')}
